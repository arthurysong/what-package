...
<img src="./images/chat/chatter.png" alt="chatter logo" width="200" align="center" />

# Problem

About a month ago, I found out about Ben Awad's discord rip off named DogeHouse. JK, lol it's totally not a rip off. Just a different take on voice streaming platform. For a couple of days, I tested out the app. 
Joined a few rooms, chatted through text and voice with people and I was so impressed by how "functional" everything was. Here was an app, created by this one guy, Ben, and there is a steady stream of 10+ users 
...
to submit a Poker turn, and the app took a noticeable chunk of time to respond to my interactions. It wasn't that slow when I would dev in my local environment, so it must be how I set up the system 
in production I thought. How fast is the computer that's running my backend server? etc...

I could easily have improved the performance by moving away from the free tier Heroku deployment. --😑 😑 😑, I should definitely spend some more money on the projects I work on, if I want any sort of a decent 
...
efficiently handle the load coming from the client? How can I make sure my app is available in all regions in the globe at optimal speeds? How can I scale stateful things like Elixir chat server 
...
I have two chat clients connecting to an Elixir backend via Websockets. The backend is setup with an Load balancer, so each client should theoretically connect to 
two different backend machines. You can see the IP address of the connected machine in the top right of the client (you might need to refresh the page to get two separate 
...
# Things I used.
* RabbitMQ Cluster on AWS Classic Load Balancer
* Elixir GenServers, Registry, Cowboy Plug
* Websockets
* AWS ECS to run my Elixir nodes (IDK why I went with ECS; I've been wanting to learn docker & container services for a while so why not :P)
* Docker
...
First, I needed a way for the Elixir instances to stay "synced" with each other. I don't think I'm going to go too much what RabbitMQ does, I mostly used the official docs to learn the basics of 
RabbitMQ.

But essentially, I used RabbitMQ as a pub/sub. I created a GenServer process that runs my RabbitMQ client in Elixir. Each node of my Elixir backend creates it's own unique queue (e.g. "chatter_queue_12glbn0892") and attaches itself to the "exchange" (e.g. "chatter_exchange")
...
That's a very high level explanation of how it works. If you're interested you should take a look at the code specifically `/lib/amqp_consumer.ex` and also `/lib/websocket/socket_handler.ex`.

Important things to note for setting up RabbitMQ:
...
I know there's Phoenix and according to the Phoenix docs, Phoenix makes it very easy to scale horizontally. But still, I wanted to learn how to manually use a message broker like 
...
process that receives the msg will send the msg to the websockets client.
...
I probably should've just used EC2 instances so I don't attach a bunch of unneccesary tech for this project, but I decided to run my nodes using docker and AWS container management
